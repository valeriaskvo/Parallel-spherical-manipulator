 ...
 */
boolean_T step(int *STEP_TYPE, double Hessian[36], d_struct_T *TrialState,
               h_struct_T *MeritFunction, c_struct_T *memspace, f_struct_T
     ...
     case 1:
      ...
      if (b_TrialState.state > 0) {
        ...
        if (oldDirIdx < MeritFunction->penaltyParam) {
          ...
          if ((MeritFunction->initFval + oldDirIdx * 0.0) - MeritFunction->phi >
    ...
    if (guard1) {
      if (TrialState->state != -6) {
        ...
        for (mIneq = 0; mIneq < 6; mIneq++) {
          ...
          Hessian[mIneq + 6 * mIneq] = oldDirIdx;
          i += mIneq;
          k = 4 - mIneq;
          if (0 <= k) {
